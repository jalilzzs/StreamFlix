  ...
  return (
    <>
      <nav className="navbar">
        ...
        <Link to="/" className="brand"><em>Stream</em>Flix</Link>
        ...
        <div className="nav-right">
          <Link to="/friends" className="icon-btn" title={t('nav_friends')} aria-label={t('nav_friends')}>
            <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" />
              <path d="M23 21v-2a4 4 0 0 0-3-3.87" /><path d="M16 3.13a4 4 0 0 1 0 7.75" />
            </svg>
          </Link>
          <button className="icon-btn" onClick={() => navigate('/settings')} title={t('settings_title')} aria-label={t('settings_title')}>
            <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      <div className={`drawer ${drawerOpen ? 'show' : ''}`}>
        ...
        <Link to="/friends" onClick={() => setDrawerOpen(false)}>{t('nav_friends')}</Link>
        <Link to="/settings" onClick={() => setDrawerOpen(false)}>{t('settings_title')}</Link>
      </div>
    </>
  );
}
